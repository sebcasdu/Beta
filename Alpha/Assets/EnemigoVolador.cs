...
using UnityEngine;
using TMPro;
public class EnemigoVolador : MonoBehaviour, Enemigo
{
    ...
    public void RecibirDaño(float d)
    {
        ...
        vida -= d;
        rb.AddForce((jugadorTransform.position - transform.position) * -250);
    ...
    public void Ataques()
    {
        ...
        transform.position = Vector3.MoveTowards(transform.position, jugadorTransform.position, 5 * Time.deltaTime);
    ...
    public void Movimiento(float f)
    {
        ...
        {
            Target.position = transform.position + new Vector3(Random.Range(-patrolLimitX, patrolLimitX), Random.Range(-patrolLimitY, patrolLimitY),0);
        }
